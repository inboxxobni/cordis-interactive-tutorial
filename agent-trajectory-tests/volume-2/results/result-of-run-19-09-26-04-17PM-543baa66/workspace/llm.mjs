// llm.mjs - the model adapter: the only plugin here that talks to the network.
//
// Provides: llm -> await ctx.llm.chat(messages, toolDefinitions)
// Injects:  nothing (ACTIVE on mount; it is a leaf of the inject graph)
//
// Credentials come ONLY from the environment (set from Settings): never from a
// config file, never asked for, never written to disk, never logged - only
// whether a key is present is ever printed.
//
// Speaks two wire formats and normalizes BOTH back to the OpenAI-shaped
// assistant message ({ role, content, tool_calls }) that agent-loop.mjs drives:
//   openai-compatible (also deepseek): POST {base}/chat/completions
...
/** Content may be a string, or an array of parts - always flatten to text. */
...
/** Tool arguments arrive as a JSON string; the loop hands tools real objects. */
...
/** {name, description, input_schema} -> provider wire format. */
...
/** Internal transcript -> Anthropic Messages API shape. */
function toAnthropic(messages) {
  ...
  for (const m of messages) {
    if (m.role === 'system') continue // hoisted out of the message list

    if (m.role === 'tool') {
      const block = { type: 'tool_result', tool_use_id: m.tool_call_id, content: textOf(m.content) }
      const last = out[out.length - 1]
      // Consecutive tool results must be grouped into ONE user turn.
      if (
        last?.role === 'user' &&
...
function fromOpenAI(data) {
  ...
  if (!message) {
    throw new Error(`llm: no message in response - ${JSON.stringify(data).slice(0, 300)}`)
  }
  return {
    role: 'assistant',
...
/** Anthropic blocks -> the same OpenAI-shaped assistant message the loop speaks. */
...
export class LLM {
  constructor() {
    ...
    // Real observability for a caller that wants to know what happened.
    this.calls = 0
    this.lastUsage = null
  }

  get configured() {
    return Boolean(MODEL && API_KEY)
  }

  /**
   * One model call.
   *
   * @param {Array} messages - internal transcript (system/user/assistant/tool).
   * @param {Array|{definitions:Array}} tools - tool definitions to expose.
  ...
  async chat(messages, tools) {
    ...
    if (IS_ANTHROPIC) {
      ...
      const wire = wireTools(definitions)
      body = {
        model: MODEL,
        max_tokens: MAX_TOKENS,
        messages: list,
        ...(wire ? { tools: wire, tool_choice: 'auto' } : {}),
      }
    }
