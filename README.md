...
**Volume 2 - "Let's build our own Coding Agent Harness from scratch"** (chapters 16-24, real): the same 9-part "how a coding agent works" curriculum this project's own sibling [`aicodingagent-ts`](../aicodingagent-ts) teaches (a real, minimal, ~4-tool coding agent, plain TypeScript, no Cordis) - re-taught by having the **connected agent build each concept itself**, as a real Cordis plugin, into `workspace/` - nothing here is pre-seeded. Chapters 17-21 ask it to write `agent-loop.mjs`, `tools.mjs`, `context-window.mjs`, `compaction.mjs`, and `system-prompt.mjs` one at a time, each mounted via `mount_plugin` right after writing it - watch `agentLoop`'s fiber (`static inject = ['tools', 'llm', 'systemPrompt']`) sit `PENDING` the whole way. Chapter 22 asks it to write the last one, `llm.mjs` (reading `process.env.CORDIS_AGENT_*`, bridged from whichever provider is already configured above - the agent never sees the raw key) and the fiber flips to `ACTIVE` live, on canvas. Chapter 23 is the payoff: a new `run_workspace_agent_turn` tool drives one real turn through the agent-loop just built - a real LLM call, real tool calls against this same shared `workspace/`. By chapter 24 the six files are a real, complete, working coding agent sitting in `workspace/` - copy it anywhere, `pnpm install && pnpm dev`, and it runs standalone. This is explicitly a minimal rehearsal for ACRYL's own real self-building capability: directing an agent to build its own Cordis plugins for its own environment.
